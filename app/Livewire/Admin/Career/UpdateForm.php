...
class UpdateForm extends Component
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            'alias'             => ['required', 'unique:careers,alias,' . $this->info->id . ',id,deleted_at,NULL', 'max:100', new TextRule(), new NoDangerousTags()],
            'description'       => ['required', new EditorRule()],
            'designation'       => ['required', 'regex:/^([a-zA-Z\s])+$/u', 'max:100', new TextRule(), new NoDangerousTags()],
            'location'          => ['required', 'regex:/^([a-zA-Z\s])+$/u', 'max:100', new TextRule(), new NoDangerousTags()],
            'salary'            => ['required', 'max:50', new TextRule(), new NoDangerousTags()],
            'job_type'          => ['required', 'regex:/^([a-zA-Z\s])+$/u', 'max:20', new TextRule(), new NoDangerousTags()],
            'short_description' => ['required', 'max:300', new TextRule(), new NoDangerousTags()],
            'meta_title'        => ['nullable', 'max:200', new TextRule(), new NoDangerousTags()],
