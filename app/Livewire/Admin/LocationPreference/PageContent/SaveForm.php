...
{
    use WithFileUploads;
    protected $listeners = ['LocationPageContentAdd' => 'mount','updateEditorValue' => 'updateEditorValue'];
    public $info, $location, $description, $description2, $serviceId, $cityId, $image;
    ...
    function getlocationData($Id){
        $this->info = \App\Models\Location::find($Id);
        $this->dispatch('generateEditorAgain');
    }

    public function render()
    {
        return view('livewire.admin.location-preference.page-content.save-form');
    }
    ...
    public function rules()
    {
        return [
            ...
            'description2' => [(($this->info->city_id ?? 0) == 0 ? 'required' : 'nullable'),new EditorRule()],
            'page_meta_title' => [(($this->info->city_id ?? 0) == 0 ? 'required' : 'nullable'), 'max:100',new TextRule()],
            'page_meta_keywords' => [(($this->info->city_id ?? 0) == 0 ? 'required' : 'nullable'), 'max:500',new TextRule()],
            'page_meta_description' => [(($this->info->city_id ?? 0) == 0 ? 'required' : 'nullable'), 'max:500',new TextRule()],
    ...
    public function UpdateForm()
    {
        ...
        $this->reset(['description', 'meta_title', 'meta_keywords', 'meta_description', 'serviceId', 'cityId', 'image']);
        $this->dispatch('LocationPageContentAdd');
