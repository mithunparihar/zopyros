<form class="pt-0 g-2" wire:submit.prevent="SaveForm" enctype="multipart/form-data">
    ...
    <div class="card p-3 mt-2">
        <div class="row">
            ...
            <div class="mb-1 col-md-12">
                <x-admin.form.label for="title" class="form-label" :asterisk="true">URL Alias</x-admin.form.label>
            ...
            <div class="mb-1 col-md-3">
                ...
                <x-admin.form.input wire:model="post_date" :min="$post_date" onClick="showPicker()" id="post_date"
                    type="date" @class(['is-invalid' => $errors->has('title')]) />
                @error('post_date')
                    <x-admin.form.invalid-error>{{ $message }}</x-admin.form.invalid-error>
                @enderror
            </div>
        ...
        <div class="row">
            ...
            <div class="col-2 mt-3">
                @if ($image)
                    ...
                    @endif
                @else
                    <x-image-preview id="blah2" class="defaultimg" imagepath="blog" width="300"
                        :image="$info->image" />
                @endif
            </div>
            <div class="col-4 mt-3">
            ...
            <div class="col-2 mt-3">
                @if ($banner)
                    ...
                    @endif
                @else
                    <x-image-preview id="blah2" class="defaultimg" imagepath="blog_banner" width="300"
                        :image="$info->banner" />
                @endif
    ...
    <div class="card p-3 mt-2">
        <div class="row">
            <div class="col-md-12">
                ...
                <span>Whatever feature field you add data to, the same data will be shown on the website.</span>
            </div>
        </div>
        @foreach ($tablecontents as $key => $input)
            <div class="row bg-secondary rounded-2 mb-3 py-2 bg-opacity-10">
                ...
                <div class="my-2 col-md-7 d-flex justify-content-end align-items-center">
                    @if ($key > 0 || !empty($tablecontents[$key]['pre_id'] ?? ''))
