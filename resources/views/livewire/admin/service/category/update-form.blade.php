<form class="pt-0 g-2" wire:submit.prevent="SaveForm" enctype="multipart/form-data">
    ...
    <div class="card p-3 mt-2">
        <div class="row">
            ...
            <div class="mb-1 col-md-12">
                <x-admin.form.label for="alias" class="form-label" :asterisk="true">URL Alias / Slug</x-admin.form.label>
                <x-admin.form.input wire:model="alias" type="text" @class(['otherValidation', 'is-invalid' => $errors->has('alias')])
                    placeholder="Enter URL Alias / Slug Here..." />
                @error('alias')
                    <x-admin.form.invalid-error>{{ $message }}</x-admin.form.invalid-error>
                @enderror
            </div>
        ...
        <div class="row">
            ...
            <div wire:loading.remove wire:target="image" class="col-2">
                @if ($image)
                    ...
                    @endif
                @else
                    <x-image-preview id="blah2" class="defaultimg" imagepath="service/category" width="200"
                        :image="$info->image" />
                @endif
            </div>
        </div>
    ...
    <div class="card p-3 mt-2">
        <div class="row">
            <div class="mb-1 col-md-12">
                ...
                <x-admin.form.input wire:model="meta_title" type="text" placeholder="Enter Meta Title Here..."
                    @class([
                        'otherValidation',
